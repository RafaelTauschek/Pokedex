...
async function loadAllPokemonData() {
    await loadPokemon();
    await loadSinglePokemon(pokemonNames);
}


async function loadPokemon() {
    let cacheKey = `pokemon_name`;
    if (localStorage.getItem(cacheKey)) {
        console.log('Fetching from cache - Name');
        pokemonNames = JSON.parse(localStorage.getItem(cacheKey));
    }
    let offset = pokemonNames.length;
    let url = `https://pokeapi.co/api/v2/pokemon/?limit=${limit}&offset=${offset}`;
    let response = await fetch(url);
    let fetchedData = await response.json();
    pokemonNames.push(...fetchedData['results']);
    localStorage.setItem(cacheKey, JSON.stringify(pokemonNames));
}


async function loadSinglePokemon(pokemonNames) {
    let cacheKey = `pokemon_details`;
    if (localStorage.getItem(cacheKey)) {
        console.log('Fetching from cache - Details');
        pokemonDetails = JSON.parse(localStorage.getItem(cacheKey));
    }
    for (let i = 0; i < pokemonNames.length; i++) {
        let url = `https://pokeapi.co/api/v2/pokemon/${pokemonNames[i]['name']}`;
        let response = await fetch(url);
        let fetchedData = await response.json();
        pokemonDetails.push(fetchedData);
        localStorage.setItem(cacheKey, JSON.stringify(pokemonDetails));
    }
}

/*
async function loadPokemonSpecies() {
    let cacheKey = 'pokemon_species';
    if (localStorage.getItem(cacheKey)) {
        console.log('Fetching from cache - Species');
        pokemonSpecies = JSON.parse(localStorage.getItem(cacheKey));
    }
    let url = `https://pokeapi.co/api/v2/pokemon-species/${}/`;
    let response = await fetch(url);
    let fetchedData = await response.json();
    pokemonSpecies.push(fetchedData);
    localStorage.setItem(cacheKey, JSON.stringify(pokemonSpecies));
}*/
